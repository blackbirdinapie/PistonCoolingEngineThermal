...
#Get the fluid mapping surfaces
for module in root.getiterator('module'):
        if (module.get('type')=="share"):
            interval = int(module.get('cycle_computation_interval'))
            
        if (module.get('type')=="piston_cooling"):
            for bc in root.getiterator('bc'):
                if (bc.get('type') == "piston" or bc.get('type') == "gallery"):
                    fluidSurfs.append(bc.get('patch'))

tree2 = ET.parse(solidSpro)
root2 = tree2.getroot()

#Get the solid mapping surfaces
for module in root2.getiterator('module'):
    if (module.get('type')=="piston_cooling"):
            for bc in root2.getiterator('bc'):
                if (bc.get('type') == "fluid_interface"):
                    solidSurfs.append(bc.get('patch'))
